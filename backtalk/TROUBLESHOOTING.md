...
- **Nothing happens when I hold the key (macOS)**: the terminal app needs **Input Monitoring** permission: System Settings → Privacy & Security → Input Monitoring → add your terminal (Terminal, iTerm, etc.), then restart the terminal. The mic prompt is separate and appears on first recording. (With `"ptt_scope": "face"` there is no global listener: the key belongs to the face page and works only while that browser tab is focused.)
...
- **It "corrects" what you said into a different, plausible sentence**: that is `small.en` guessing from context on words it did not catch cleanly; nothing in backtalk rewrites transcripts. Set `"stt_model": "large-v3-turbo"` (and `"stt_quant": ""` on Apple Silicon, where the 8-bit conversion is slower): about half the errors of `small.en` on unclear speech, for ~0.9 s more per utterance and ~1.5 GB of memory. `medium.en` is the middle step. The numbers are in `backtalk/config.py`.
