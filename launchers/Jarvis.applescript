...
on quit
	...
	--
	-- ONLY IF ITERM IS STILL UP. `tell application "iTerm" to close win`
	-- LAUNCHES iTerm when it is not running -- that is what `tell` does, and
	-- a `try` cannot stop it, because the launch is not the error. So the
	-- most ordinary way to shut down (close the window, iTerm quits with its
	-- last one, the agent dies, idle notices and quits us) ended by starting
	-- iTerm back up for a window we no longer had, and it opened a fresh
	-- empty one on the way in. Closing the terminal spawned a terminal.
	-- `running of` is the one property that answers without launching, which
	-- is why `on run` already uses it.
	if win is not missing value then
		if running of application "iTerm" then
			try
