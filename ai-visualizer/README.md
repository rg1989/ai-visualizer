...
The face doubles as a display your agent can draw on. The glass is a transparent 12×8 grid over the face where the agent manifests cards on demand — a note, a map, a countdown, a calendar of your week — with animated entrances and exits and one consistent look. Ask to *see* something and the answer lands on screen instead of arriving as a wall of spoken text.

The agent drives it with two small scripts in `bin/`:

```
bin/glass.sh '{"a":"show","type":"map","q":"Aleppo"}'
bin/glass-state.sh
```

`glass.sh` sends one verb per call — show, update, move, pin, dismiss, clear — and prints the server's reply. `glass-state.sh` prints what is up, where it sits, how long each card has left, and where the next one would land. The server owns the geometry: leave out the cell and it picks a free spot clear of the face's own chrome; when a placement can't happen it refuses out loud, naming what's in the way, instead of silently rearranging your screen. Cards fade after a few minutes unless pinned, so the glass tidies itself.

Nine card types ship — note, image, map, calendar, timer, list, iframe, a sandboxed html escape hatch, and **player** — all wearing the same glass-panel chrome and theme tokens, so whatever goes up looks like one system. It all rides the same tiny stdlib server and the same `/state` poll the faces already make: no new port, no build step, no dependencies, and mutations are accepted from this machine only.

### The player

Ask for a song and the glass puts up a Winamp-shaped deck — LED time, scrolling
title, spectrum, transport, seek, volume, and a playlist editor — wearing the
current theme's colours, so it reads as SHODAN green or JARVIS sky-blue
without a second skin.

```
bin/glass.sh '{"a":"show","type":"player","q":"Sultans of Swing"}'
bin/glass.sh '{"a":"show","type":"player","q":"lo-fi beats","count":12}'
bin/glass.sh '{"a":"show","type":"player","playlist":"PLgaFNC_I_Zkl..."}'
bin/glass.sh '{"a":"show","type":"player","q":"tiny desk concert","mode":"video"}'
```

`q` searches YouTube and takes the top hit (`count` queues more); `"<artist> -
Topic"` channels sort first, because those are the auto-generated uploads
YouTube Music itself serves, so a song request lands on the track rather than a
live cover. `playlist` takes a YouTube playlist id and lets YouTube own the
queue. `tracks` takes explicit `{"id","title","artist","dur"}` entries when you
already know them. `mode: "video"` gives the embed the top of the card and keeps
the transport strip underneath.

**No API key, and no account.** The search reads the same public results page a
logged-out browser gets, and playback is a normal cross-origin YouTube embed.
That buys "play me a song" with zero setup. It does not buy your library, your
likes, or a Premium ad-free stream — those need an OAuth flow this server
deliberately does not have. Whether the embed itself is signed in is your
browser's third-party-cookie setting talking, not this code. Videos whose owner
disabled embedding are skipped by YouTube's own player; a queue simply moves to
the next one.

The spectrum is CSS, not audio: a cross-origin embed's samples are unreachable
from the page, so the bars run while the player reports playing and freeze when
it doesn't.

Want a look without wiring anything? Add `?glassdemo=1` to any face URL and the overlay stages one sample card of each type, no server needed. Turn the layer off for good with `"glass": false` in the config.
...
- **Add a theme.** Same rule, one folder over: drop a folder into `themes/` with a `theme.js` that calls `AVTheme.add()` — a palette, the canvas colours, its own fonts and images — and it is in the theme pickers on the next reload. Nothing lists the themes you have, so a theme stays private simply by staying out of the repo (`.gitignore` has the two lines). `themes/README.md` is the contract, and zipping the folder is how you hand one to someone else.
