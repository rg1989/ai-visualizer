...
# they are caught by length, because every one of them qualifies.
_TYPING_KEYS = {
    "enter", "return", "space", "tab", "backspace", "delete", "esc",
    "escape", "up", "down", "left", "right", "page_up", "page_down",
    "home", "end",
}


def _warn_if_typing_key(name: str):
    """THE LISTENER IS GLOBAL -- it does not care which window has focus.

    Bind the talk key to something you type with and every press anywhere on
    the machine opens the mic, and a press DURING a reply interrupts it (see
    the module docstring). Field-caught with ptt_key "enter": a log full of
    "[ptt] (tap or empty -- ignored)" and answers cut off mid-sentence every
    time the owner typed in another application.

    Checked BEFORE the single-character early return, which used to skip it:
    a printable character is by definition typed constantly, so the very
    worst bindings were the only silent ones. Warn, but obey -- it is their
    keyboard. "home" is in the list and is also the default, so it warns on
    a key that is genuinely fine for most people; that is the right side to
    err on for a setting whose failure mode is a mic nobody asked for.
    """
    if len(name) == 1 or name in _TYPING_KEYS:
        what = "character" if len(name) == 1 else "key"
        print(f"[ptt] WARNING: talk key is {name!r}, and the listener is "
              f"GLOBAL -- every time you press that {what} in ANY app it "
              f"opens the mic, and it interrupts a reply in progress. Pick "
              f"one you never type with (f13, right_option) via ptt_key in "
              f"backtalk.json.", flush=True)


def resolve_key(name: str):
    """'home' / 'f13' / 'right_alt' / any single character -> pynput key."""
    name = (name or "home").strip().lower()
    _warn_if_typing_key(name)
    if len(name) == 1:
