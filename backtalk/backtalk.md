...
2. **The key.** Default is `home`. Ask what they want to hold to talk (in hands-free listening it is still the interrupt): a key they never type with is best (`home`, `end`, `f13`–`f19`, `right_alt`). Set `ptt_key`. If they keep the face open in a browser and typing that key elsewhere must never open the mic, set `"ptt_scope": "face"` too: the key then counts only while the face's tab has focus, and no Input Monitoring is needed.
