...
- **Wake word mode** (the third microphone mode): always listening like hands-free, but nothing reaches the agent unless it starts with a wake phrase — the agent's name and the natural greetings ("Jarvis", "hey Jarvis", "hi/hello/okay Jarvis") by default, derived from the configured name. Say the name alone and a rising chime answers: you have `wake_window_s` seconds (default 8) to speak your request, closed by a falling chime, then the reply. Or say it in one breath — "Jarvis, what's the weather" — and only the falling chime plays before the answer. Everything else said in the room is transcribed locally and dropped, never sent anywhere. Whisper sometimes misspells names; `wake_aliases` lists accepted variants. Saying the name mid-reply interrupts it, exactly like the talk key; a window that closes on silence says so with a low third chime. **After the agent answers, the mic stays hot for a few seconds** (`wake_followup_s`, default 7): follow-ups need no wake word, each exchange re-opens the window, and silence closes it — so one "hey Jarvis" carries a whole conversation, and turning to talk to someone else simply lets it lapse. **The face shows the truth at all times**: a corner badge reads OPEN MIC (red), WAKE WORD / WAKE · LIVE (amber, bright while the window is hot), or PUSH TO TALK (green), and disappears when the voice line is down. **Launching from a terminal offers a one-keypress mode menu** (1 open, 2 wake, 3 push to talk, ten seconds, silence keeps the saved default — so auto-starts sail through; `mode_select_on_launch: false` disables). Switch live with "wake word mode"; a pending spoken permission ask never needs the wake word, and neither does an exact quit phrase ("goodbye Jarvis" — exact, so room speech merely containing "hang up" stays ignored). Chimes live in `assets/wake.wav` / `done.wav` / `close.wav` (restyle with `assets/make_chimes.py`).
- **Speaker identification (optional, local).** With `"voice_id": true` and voices enrolled, every mic turn reaches the agent tagged with who spoke (`[voice: Alex] ...`), so a household agent can personalize — whose reminder, whose preferences, who asked what. Enroll each person by just saying **"learn my voice"** in a live session — Jarvis asks the name, walks you through four sentences out loud, checks they agree, and the profile is effective immediately (re-enrollment is the same phrase). A terminal alternative exists for headless setups: `.venv/bin/python -m backtalk.enroll <Name>`, run while the voice line is stopped. Prints live in `voices.json`, the model (SpeechBrain ECAPA, ~80 MB, fetched from Hugging Face on first use) caches in `models/`, and after that download nothing leaves the machine. (Windows: same feature, `.venv\Scripts\python -m backtalk.enroll <Name>`.) Unrecognized voices are tagged honestly rather than guessed. **Personalization only, never authorization** — a recording of a voice IS that voice to the model; approvals stay with the spoken permission gate. Tune `voice_id_threshold` if guests get mislabeled (raise) or owners go unrecognized (lower); re-enrollment fixes a drifted print.
...
- **The talk key needs a global key listener.** For the key to work when the voice line isn't your focused window, the process has to watch keyboard events system-wide. `backtalk/ptt.py` compares each event against the one key you configured and discards the rest. It stores nothing and writes nothing anywhere. Ninety-one lines, so you can read all of it in a minute. macOS asks for Input Monitoring permission before it will run, which is the OS telling you what the program can see. Rather not? Set `"ptt_scope": "face"` in backtalk.json: the face page reports the key itself, so it works only while the face's browser tab has focus, Enter typed into any other window stays that window's Enter, and no Input Monitoring is needed.
